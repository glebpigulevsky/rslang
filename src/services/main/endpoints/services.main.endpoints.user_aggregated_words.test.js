...
} from '../../common/services.common.constants';
...
describe('get user aggregated words by group', () => {
  ...
  it('should return words collection"', async () => {
    ...
    const res = await userAggregatedWords.getAllUserAggregatedWords(
      { group: 1 },
...
describe('get user aggregated words with filter difficulty = hard', () => {
  ...
  it('should return words collection"', async () => {
    ...
    userAggregatedWords._apiService = new ApiService(MAIN_API_URL);
    const res = await userAggregatedWords.getAllUserAggregatedWords(
      { group: 0, filter: USER_AGGREGATED_WORDS_FILTER.byDifficultyHard },
      { userId: auth.userId, token: auth.token },
    );
    expect(res).toBeDefined();
...
describe('get user aggregated words with filter difficulty = easy and optional.repeat = true', () => {
  ...
  it('should return words collection"', async () => {
    ...
    userAggregatedWords._apiService = new ApiService(MAIN_API_URL);
    const res = await userAggregatedWords.getAllUserAggregatedWords(
      { group: 0, filter: USER_AGGREGATED_WORDS_FILTER.byDifficultyHardAndRepeat },
      { userId: auth.userId, token: auth.token },
    );
    expect(res).toBeDefined();
...
describe('get user aggregated words with filter difficulty = easy and optional.repeat = true', () => {
  ...
  it('should return words collection"', async () => {
    ...
    userAggregatedWords._apiService = new ApiService(MAIN_API_URL);
    const res = await userAggregatedWords.getUserAggregatedWord(
